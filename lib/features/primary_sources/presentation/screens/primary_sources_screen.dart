...
class _PrimarySourcesScreenState extends State<PrimarySourcesScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<PrimarySourcesExpansionCubit>(
      create: (_) => PrimarySourcesExpansionCubit(),
      child: BlocListener<PrimarySourcesCubit, PrimarySourcesState>(
        listenWhen: (previous, current) =>
            previous.full != current.full ||
            previous.significant != current.significant ||
            previous.fragments != current.fragments,
        listener: (listenerContext, state) {
          final knownIds = <String>{
            ...state.full.map((source) => source.id),
            ...state.significant.map((source) => source.id),
            ...state.fragments.map((source) => source.id),
          };
          listenerContext.read<PrimarySourcesExpansionCubit>()
            ..retainKnownSourceIds(knownIds);
        },
        child: Builder(
          builder: (screenContext) {
            ...
            final PrimarySourcesState state = screenContext.select(
            ...
            if (isLoading &&
              ...
              return Scaffold(
                appBar: AppBar(
                  title: Text(
                    AppLocalizations.of(screenContext)!.primary_sources_screen,
                  ),
  ...
  Widget _buildSourceItem(BuildContext context, PrimarySource source) {
    return Builder(
      builder: (itemContext) {
        final expanded = itemContext.select(
          (PrimarySourcesExpansionCubit cubit) => cubit.isExpanded(source.id),
        );
        return SourceItemWidget(
          source: source,
          showMore: expanded,
          onToggleShowMore: () {
            itemContext.read<PrimarySourcesExpansionCubit>().toggle(source.id);
          },
        );
      },
    );

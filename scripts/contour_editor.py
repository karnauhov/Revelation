#!/usr/bin/env python3
"""Standalone contour editor for Revelation manuscript pages."""
...
def default_primary_sources_dir() -> Path:
    # Mirrors app logic: getApplicationDocumentsDirectory()/revelation/primary_sources
    ...
    for i, ch in enumerate(body):
        if ch == "[":
            ...
            if depth == 0 and block_start is not None:
                blocks.append(body[block_start:i])
                block_start = None
    return blocks


def parse_verse_snippet(snippet: str) -> dict:
    ...
    word_indexes = [int(v) for v in re.findall(r"\d+", word_indexes_body)]
...
class ContourEditor(tk.Tk):
    def __init__(self, start_dir: Path, image: Path | None = None) -> None:
        super().__init__()
        self.title("Contour Editor")
        ...
        self.chapter = tk.IntVar(value=1)
        self.verse = tk.IntVar(value=1)
        self.word_indexes = tk.StringVar(value="")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(top, text="Open Image", command=self.open_image).grid(row=0, column=0, padx=2)
        ttk.Button(top, text="New Contour", command=self.new_contour).grid(row=0, column=1, padx=2)
        ttk.Button(top, text="Clear Contour", command=self.clear_contour).grid(row=0, column=2, padx=2)
        ttk.Button(top, text="Delete Contour", command=self.delete_contour).grid(row=0, column=3, padx=2)
        ttk.Button(top, text="Pick Label", command=self.pick_label).grid(row=0, column=4, padx=8)
        ttk.Button(top, text="Copy Contours", command=self.copy_contours).grid(row=0, column=5, padx=2)
        ttk.Button(top, text="Copy Verse", command=self.copy_verse).grid(row=0, column=6, padx=2)
        ttk.Button(top, text="Import Verse", command=self.import_verse_dialog).grid(row=0, column=7, padx=2)
        ttk.Checkbutton(top, text="Snap axis", variable=self.snap_axis).grid(row=0, column=8, padx=8)

        mode_frame = ttk.Frame(top)
        mode_frame.grid(row=0, column=9, padx=8)
        ...
        for col, (title, value) in enumerate([("Add", "add"), ("Move", "move"), ("Delete", "delete")], start=1):
            ttk.Radiobutton(mode_frame, text=title, value=value, variable=self.mode).grid(row=0, column=col, padx=2)

        meta = ttk.Frame(top)
        meta.grid(row=1, column=0, columnspan=12, sticky="ew", pady=(8, 0))
        ttk.Label(meta, text="Chapter").grid(row=0, column=0, padx=(0, 4))
        ttk.Spinbox(meta, from_=1, to=999, textvariable=self.chapter, width=5).grid(row=0, column=1)
        ttk.Label(meta, text="Verse").grid(row=0, column=2, padx=(8, 4))
        ttk.Spinbox(meta, from_=1, to=999, textvariable=self.verse, width=5).grid(row=0, column=3)
        ttk.Label(meta, text="Indexes").grid(row=0, column=4, padx=(8, 4))
        ttk.Entry(meta, textvariable=self.word_indexes, width=56).grid(row=0, column=5, sticky="ew")
        ttk.Label(meta, textvariable=self.label_text).grid(row=0, column=6, padx=(10, 0), sticky="w")
    ...
    def load_image(self, path: Path) -> None:
        ...
        self.title(f"Contour Editor - {path}")
        self.fit_view()
    ...
    def rel_to_canvas(self, p: Point) -> Point:
        iw, ih = self.image_size
        return (self.pan_x + p[0] * iw * self.zoom, self.pan_y + p[1] * ih * self.zoom)
    ...
    def on_left_down(self, event: tk.Event) -> None:
        ...
        if self.wait_label_pick:
            ...
            self.label_text.set(f"label: Offset({rel[0]:.4f}, {rel[1]:.4f})")
            self.redraw()
            return
        mode = self.mode.get()
        if mode == "add":
    ...
    def on_wheel_win(self, event: tk.Event) -> None:
        self.zoom_at(event.x, event.y, 1.1 if event.delta > 0 else 1 / 1.1)

    def on_wheel_linux(self, event: tk.Event) -> None:
    ...
    def zoom_at(self, x: float, y: float, factor: float) -> None:
        ...
        if abs(self.zoom - old) < 1e-8:
            return
        self.pan_x = x - (x - self.pan_x) * (self.zoom / old)
        self.pan_y = y - (y - self.pan_y) * (self.zoom / old)
        self.redraw()
    ...
    def import_verse_dialog(self) -> None:
        ...
        ttk.Label(
            ...
            text=(
                ...
                "Expected fields: chapterNumber, verseNumber, labelPosition, "
                "wordIndexes, contours."
            ),
            justify="left",
        ...
        def do_import() -> None:
            ...
            if not raw:
                messagebox.showwarning("Import Verse", "Paste Verse(...) first.")
                return
            try:
    ...
    def apply_imported_verse(self, payload: dict) -> None:
        ...
        self.label_text.set(
            f"label: Offset({self.label_position[0]:.4f}, {self.label_position[1]:.4f})"
        )

        self._refresh_contour_info()
        self.redraw()
        self.status.set(
            f"Imported Verse {chapter}:{verse} with {len(contours)} contour(s)."
        )
    ...
    def _refresh_contour_info(self) -> None:
        if not self.contours:
            self.contours = [[]]
            self.active_contour = 0
        summary = ", ".join(f"{i + 1}:{len(c)}" for i, c in enumerate(self.contours))
        self.status.set(f"Contours points count [{summary}] | active={self.active_contour + 1}")

    def redraw(self) -> None:
        self.canvas.delete("all")
        if self.image is None:
            self.canvas.create_text(self.canvas.winfo_width() / 2, self.canvas.winfo_height() / 2, text="Open image", fill="#ccc")
            return
        iw, ih = self.image_size
        dw, dh = max(1, int(iw * self.zoom)), max(1, int(ih * self.zoom))
        key = (str(self.image_path), dw, dh)
        if key != self.image_cache_key:
        ...
        for ci, contour in enumerate(self.contours):
            if len(contour) < 1:
                continue
            color = "#00a06f" if ci == self.active_contour else "#0b8561"
            pts = [self.rel_to_canvas(p) for p in contour]
            if len(pts) >= 2:
                flat = [xy for p in pts for xy in p]
                self.canvas.create_line(*flat, fill=color, width=3 if ci == self.active_contour else 2)
                if len(pts) >= 3 and contour[0] != contour[-1]:
                    self.canvas.create_line(*pts[-1], *pts[0], fill=color, width=2, dash=(4, 4))
            for idx, (x, y) in enumerate(pts):
                r = 4 if ci == self.active_contour else 3
        ...
        if self.label_position:
            ...
            self.canvas.create_text(lx + 9, ly, text=f"{self.chapter.get()}:{self.verse.get()}", fill="#ffe066", anchor="w", font=("Segoe UI", 11, "bold"))

    def filtered_contours(self) -> list[list[Point]]:
        result: list[list[Point]] = []
        for contour in self.contours:
            pts = contour[:-1] if len(contour) >= 2 and contour[0] == contour[-1] else contour
            if len(pts) >= 3:
                result.append(pts)
        return result
    ...
    def copy_contours(self) -> None:
        ...
        if not contours:
            messagebox.showwarning("Copy", "Need at least one contour with 3+ points.")
            return
    ...
    def copy_verse(self) -> None:
        ...
        try:
            ...
            messagebox.showerror("Indexes", str(exc))
            return
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Standalone contour editor. Default open folder: Documents/revelation/primary_sources."
    )
    parser.add_argument("--image", type=Path, default=None, help="Image to open at startup")
    parser.add_argument("--start-dir", type=Path, default=default_primary_sources_dir(), help="Initial folder for file dialog")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    app = ContourEditor(start_dir=args.start_dir, image=args.image)
    app.mainloop()

...
- `DBManager` is now constrained to infra runtime usage, but still stores aggregated caches with mixed responsibilities.
...
| `RAD-04` | P1 | Continue `DBManager` decomposition to domain-specific data gateways | `DBManager` still keeps aggregated table caches and mixed read APIs | Introduce dedicated gateways/data-sources (`articles`, `lexicon`, `primary_sources`) and simplify runtime adapter role | `DBManager` no longer hosts feature-level aggregate caches or is reduced to composition-only adapter |
| `RAD-05` | P2 | Expand integration smoke coverage for critical features | Current smoke suite is limited to selective manual scenarios | Add smoke for `about`, `download`, and at least one cross-feature navigation flow | `integration_test/smoke` covers at least `4` scenarios and remains manual-triggered |

# Phase 3.7 - State Migration Matrix (RU)
...
## 1. Purpose
Определить и утвердить migration matrix для Phase 3.7: `feature -> cubit/bloc set -> owner state contracts`.

## 2. Approval Rules
- Базовый паттерн: `Cubit` для state-driven сценариев.
- `Bloc` допускается только для сложной event orchestration между несколькими cubit-срезами.
- Все state-контракты - immutable DTO.
- После завершения Phase 3.7 в runtime-коде не остается `provider`/`ChangeNotifier`.
...
| `topics/catalog` (`MainScreen`, `TopicList`, icons) | `MainViewModel` + локальный state в `FutureBuilder` | `TopicsCatalogCubit` | `TopicsCatalogState { String language; List<TopicInfo> topics; Map<String, CommonResource?> iconByKey; bool isLoading; AppFailure? failure; }` |
| `topics/content` (`TopicScreen`) | локальный state в `FutureBuilder` | `TopicContentCubit` | `TopicContentState { String route; String language; String name; String description; String markdown; bool isLoading; AppFailure? failure; }` |
...
| `primary_source/detail/session` | `PrimarySourceViewModel` (подмножество state) | `PrimarySourceSessionCubit` | `PrimarySourceSessionState { PrimarySource source; model.Page? selectedPage; String imageName; bool isMenuOpen; }` |
...
| `primary_source/detail/cross-slice orchestration` | внутри одного `PrimarySourceViewModel` | `PrimarySourceCoordinatorBloc` (опционально, если нужен) | `PrimarySourceCoordinatorState { bool initialized; AppFailure? failure; }` |
| `download` | stateless screen (без state-holder) | `No Cubit/Bloc in Phase 3.7` | `N/A` |
...
## 4. Notes
- В текущем scope `download` остается stateless и не требует отдельного state-holder.
- `PrimarySourceCoordinatorBloc` вводится только при подтвержденной необходимости orchestration между cubit-срезами.
- Конвенция нейминга файлов в target state-слое: `*_cubit.dart`, `*_state.dart`, `*_bloc.dart` (bloc только при необходимости).
